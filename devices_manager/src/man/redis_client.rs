...
impl RedisClient {
    ...
    pub async fn get_pubsub(&self) -> DeviceResult<redis::aio::PubSub> {
        Ok(self.client.get_async_pubsub().await?)
    }

    pub async fn get_multiplexed_conn(&self) -> DeviceResult<redis::aio::MultiplexedConnection> {
...
impl RedisRecv {
    ...
    pub async fn subscribe(&mut self, topic: &str) -> DeviceResult {
        match self.conn.subscribe(topic).await {
            ...
            Err(e) => {
                if e.is_io_error() {
                    self.reconnect()
                        .await
                        .ok_or(DeviceError::Connect("redis reconnect".to_string()))?;
                    self.conn.subscribe(topic).await?;
                    return Ok(());
                }
                Err(DeviceError::Redis(e))
            }
        }
    }

    pub async fn unsubscribe(&mut self, topic: &str) -> DeviceResult {
