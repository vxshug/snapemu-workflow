...
use crate::man::Id;
use crate::protocol::mqtt::{ConnectionId, Filter, RouterId, Topic};
use crate::protocol::mqtt::server::alert::Alert;
use crate::protocol::mqtt::version::{ConnAck, ConnAckProperties, Disconnect, DisconnectProperties, Packet, PingResp, PubAck, PubAckProperties, PubComp, PubCompProperties, PubRec, PubRecProperties, PubRel, PubRelProperties, Publish, PublishProperties, SubAck, SubAckProperties, UnsubAck};
...
pub type AuthHandler = Arc<
    dyn Fn(MqttAuthRequest) -> Pin<Box<dyn std::future::Future<Output = Result<MqttAuth, link::Error>> + Send>>
    + Send
    + Sync,
>;
...
impl MqttAuthRequest  {
    pub fn new(common_name: Option<String>, username: String, password: String, client_id: String) -> Self {
        Self {
            common_name,
            username,
            password,
